...
public abstract class PokemonCommon
{
	...
	protected boolean handleKeyInput(Key key)
	{
		ActionResult result;
		if(dialogue!=null)
			result = getDialogueDisplay().handleKeyInput(level, player, dialogue, fight, key);
		else if(fight!=null)
			result = getFightDisplay().handleKeyInput(level, player, null, fight, key);
		else
			result = getGameDisplay().handleKeyInput(level, player, null, null, key);
	...
	protected abstract KeyHandlingDisplay getFightDisplay();

	protected abstract KeyHandlingDisplay getGameDisplay();
}
